...
async def get_result(task_id: str):
    ...
    if result.state == "SUCCESS":
        return ResultResponse(task_id=task_id, status=result.state, data=result.result)

    return ResultResponse(task_id=task_id, status=result.state, error=str(result.info))
